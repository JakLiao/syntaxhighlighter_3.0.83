...
/*
enum是Java中一个特殊的类
Java提供了一个static方法values()返回一个数组包含所有声明的该枚举
类型的值并且按照声明时的顺序。
for(type var : array)
表示循环遍历每一个array中的值，也就是将weekday.values()返回的数组
中的值一个一个赋予变量k，然后执行循环体。
*/
...
{   
	...
	{   
		int i,a;
		weekday imasyuu=weekday.sun;//imasyuu显示输出起始位置为sunday
		weekday j;
		j=weekday.mon;//本月1号为星期1
